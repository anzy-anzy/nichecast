...
export async function POST(req) {
  ...
  const { character_id, title, script, scenario, resolution, duration } = await req.json();
  if (!script) return NextResponse.json({ error: 'A script (what they should say/do) is required.' }, { status: 400 });
  const info = getDb()
    .prepare(
      'INSERT INTO creator_videos (user_id, character_id, title, script, scenario, resolution, duration) VALUES (?, ?, ?, ?, ?, ?, ?)'
    )
    .run(
      user.id,
      character_id || null,
