...
export const ayrshareProvider = {
  ...
  async publish({ account, content, mediaUrl, aiGenerated }) {
    ...
    // Required disclosure for AI-generated video — avoids TikTok/Meta reach
    // suppression for undisclosed synthetic media.
    if (aiGenerated && mediaUrl) {
      body.isVideo = true;
      body.tikTokOptions = { ...(body.tikTokOptions || {}), isAiGenerated: true };
      body.instagramOptions = { ...(body.instagramOptions || {}), isAiGenerated: true };
    }
    // Optional: multi-client support via profile keys (Business plan)
    const headers = {
      'content-type': 'application/json',
      authorization: `Bearer ${envOrSetting('AYRSHARE_API_KEY')}`,
    };
