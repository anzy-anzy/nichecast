...
import { outstandProvider } from './outstand';
import { getDb } from '../db';
import { envOrSetting } from '../settings';

export function getProvider() {
  const name = (envOrSetting('POSTING_PROVIDER') || 'mock').toLowerCase();
  if (name === 'blotato') return blotatoProvider;
  if (name === 'ayrshare') return ayrshareProvider;
  if (name === 'postforme') return postForMeProvider;
  if (name === 'outstand') return outstandProvider;
  return mockProvider;
}
...
export async function processDuePosts() {
  ...
  for (const post of due) {
    ...
    // These platforms reject text-only posts — they require an image or video
    const NEEDS_MEDIA = ['instagram', 'tiktok', 'youtube', 'pinterest'];

    const outcomes = [];
    for (const account of accounts) {
      if (!mediaUrl && NEEDS_MEDIA.includes(account.platform)) {
        outcomes.push({
          platform: account.platform,
          ok: true,
          skipped: true,
          detail: `${account.platform} requires an image or video — text-only post skipped (posted to other platforms)`,
        });
        continue;
      }
      try {
        const r = await provider.publish({ account, content: post.content, mediaUrl, aiGenerated: !!post.ai_generated });
