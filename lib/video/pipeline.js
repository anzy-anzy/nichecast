...
export async function processVideoJobs({ maxJobs = 1 } = {}) {
  ...
  for (const job of jobs) {
    ...
    try {
      ...
      // 2. Voice-over — cloned voice (if selected) beats stock TTS
      setStep(db, job.id, 'Generating voice-over');
      const audioPath = path.join(workDir, 'voice.mp3');
      let clonedVoiceId = null;
      if (job.voice_id) {
        const v = db.prepare('SELECT * FROM voices WHERE id = ? AND user_id = ?').get(job.voice_id, job.user_id);
        if (v?.status === 'ready') clonedVoiceId = v.provider_voice_id;
      }
      await generateVoiceover({ script, voice: job.voice, outPath: audioPath, clonedVoiceId, userId: job.user_id });
      ...
      db.prepare(
        ...
        `/api/media/videos/${videoName}`,
        `/api/media/videos/${thumbName}`,
        seo.title || '',
        seo.description || '',
      ...
      if (job.autopost) {
        ...
        db.prepare(`INSERT INTO posts (user_id, content, media_path, account_ids, scheduled_at, ai_generated) VALUES (?, ?, ?, ?, datetime('now'), 1)`)
          .run(
            job.user_id,
