...
// Builds an AI-video generation prompt inspired by a reference video's
// title/style — for Marketing Studio's "make one like this" (real video,
// not just an idea). Deliberately not a copy.
export async function generateSimilarVideoPrompt({ refTitle, refAuthor, notes }) {
  const text = await claude(
    `A creator wants an AI-generated video ad/clip similar in style and energy to this existing video:
Title: "${refTitle}"${refAuthor ? ` by ${refAuthor}` : ''}
${notes ? `Extra context from the creator: ${notes}` : ''}

Write ONE vivid AI video-generation prompt (for a text-to-video model) capturing the same visual style, mood, pacing and camera feel — but an original scene, not a copy. Describe subject, setting, camera movement, lighting.
Respond with the prompt text only, max 400 characters.`,
    400
  );
  if (text === null) {
    return `Cinematic video inspired by "${refTitle}", similar mood and pacing, original scene, smooth camera movement, professional lighting`;
  }
  return text.trim();
}
