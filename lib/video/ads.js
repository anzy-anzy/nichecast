...
async function generateSingleAd({ job, workDir, outPath, vertical }) {
  ...
  const buf = hasFalKey()
    ? await falGenerateVideo({ prompt: job.prompt, resolution: job.resolution, duration: job.duration, imageUrl: dataUri(photos[0]) })
    : null;
  if (buf) {
...
async function generateSimilar({ job, workDir, outPath, vertical }) {
  ...
  const buf = hasFalKey() ? await falGenerateVideo({ prompt: job.prompt, resolution: job.resolution, duration: job.duration }) : null;
  if (buf) {
    fs.writeFileSync(outPath, buf);
    return 'fal.ai';
  }
