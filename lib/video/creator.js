  ...
  for (const job of jobs) {
    ...
    try {
      ...
      if (hasFalKey()) {
        ...
        buf = await falGenerateVideo({ prompt, model: job.model, resolution: job.resolution, duration: job.duration, imageUrl });
      }
      ...
      if (job.credits_charged > 0) refundCredits(job.user_id, job.credits_charged, `refund: creator video #${job.id} failed`);
