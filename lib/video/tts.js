...
export async function generateVoiceover({ script: rawScript, voice = 'onyx', outPath, clonedVoiceId, userId }) {
  const script = applyPronunciations(rawScript, userId);
  if (clonedVoiceId) {
    try {
  ...
  const key = envOrSetting('OPENAI_API_KEY');
  if (key) {
